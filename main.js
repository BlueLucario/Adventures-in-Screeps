...
module.exports.loop = () => {
    ...
    for (var thisRoom in Game.rooms) {
        ...
        if (Game.spawns[thisSpawn].store[RESOURCE_ENERGY] >= energyAtRoomLvl) {
            ...
            if (currentHarvesters < minimumNoOfHarvesters) {
                ...
                name = Game.spawns[thisSpawn].createCreep(
                    ...
                    { role: "roadworker", working: false, target: target, homeRoom: thisRoom, mySpawn: thisSpawn}
                )
            } else if (currentBuilders < minimumNoOfBuilders) {
                name = Game.spawns[thisSpawn].createCreep(
                    [WORK, CARRY, CARRY, MOVE, MOVE],
                    "Builder: " + Game.time,
                    { role: "builder", working: false, target: target, homeRoom: thisRoom, mySpawn: thisSpawn}
                )
            } else if (currentRepairers < minimumNoOfRepairers) {
                name = Game.spawns[thisSpawn].createCreep(
                    [WORK, CARRY, CARRY, MOVE, MOVE],
                    "Repairer: " + Game.time,
                    { role: "repairer", working: false, target: target, homeRoom: thisRoom, mySpawn: thisSpawn}
                )
            } else if (hostiles) {
                name = Game.spawns[thisSpawn].createCreep(
